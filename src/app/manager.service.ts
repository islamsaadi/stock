...
export class ManagerService {
  ...
  private urlAPI: string = "http://localhost:8000/manager";
  ...
  addWorker(account: any): Observable<any>{
      return this.http.post(this.urlAPI + "/addWorker",account).map(
        (resp: Response) => resp.json()
      ).catch((error:any) => Observable.throw(error.json).error || 'Add Worker failed');
  }

  companyReport(): Observable<any>{
      return this.http.get(this.urlAPI + "/report").map(
        (resp: Response) => resp.json()
      ).catch((error:any) => Observable.throw(error.json).error || 'Get Company Report failed');
    }
}
